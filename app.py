...
def pick_note_names(root: str, intervals):
    """
    Given a root note name and intervals in semitones,
    return note names, using flats when the key is a flat key.
    """
...
def beat():
    """
    Returns a random rhythmic pattern as JSON.
    1 = hit, 0 = rest.
    """
    ...
    return jsonify({
        ...
        "pattern": pattern,
        "description": "Random pattern where 1 = hit and 0 = rest"
    })

@app.route("/chord")
def chord():
    """
    Returns a random chord, covering many common chord types.
    """
...
def scale():
    """
    Returns a random scale or mode, including flats when relevant.
    """
    root = random.choice(list(NOTE_TO_INDEX.keys()))
...
def progression():
    ...
    for symbol in progression:
        ...
        chord_notes = pick_note_names(chord_root, CHORD_TYPES[chord_type])
